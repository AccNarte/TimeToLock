...
async function main() {
  console.log("🚀 Starting FileLockFactory deployment...\n");
  ...
  // Deploy FileLockFactory
  console.log("📦 Deploying FileLockFactory...");
  const Factory = await ethers.getContractFactory("FileLockFactory");
  const factory = await Factory.deploy();
  await factory.waitForDeployment();

  const factoryAddress = await factory.getAddress();
  console.log(`✅ FileLockFactory deployed to: ${factoryAddress}`);
  ...
  const deploymentFile = path.join(
    deploymentsDir,
    `filelock-${network.name}-${network.chainId}.json`
  );
  ...
  console.log(`   NEXT_PUBLIC_FILE_LOCK_FACTORY_POLYGON=${factoryAddress}`);
  console.log("\n✨ Deployment complete!");
}
